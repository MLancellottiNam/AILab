/* ============================================
   Smart Dispatch — Capa de IA del front (dos clientes)
   --------------------------------------------
   Único archivo de contacto del browser con la IA. NUNCA habla con Anthropic
   directo: siempre vía Edge Function (la API key es del servidor). Dos usos,
   dos proxies, mismo principio (parseo defensivo + fallback sin IA):

   1) aiCall(task, payload) → ai-proxy      [builder: write-doc, extract-brand…]
   2) SDIntent.classifyIntent(...) → claude-proxy  [onboarding: intención libre]

   REGLA DE ORO: quien llama decide el fallback. Sin IA / error / timeout →
   camino manual, nunca pantalla rota.
   ============================================ */

/* -----------------------------------------------------------
   1) Cliente del builder — aiCall(task, payload) → ai-proxy
   Resuelve con `data` (ya parseado por el proxy) si {ok:true}; LANZA en
   cualquier fallo (no-ok, red, timeout, JSON inválido). El builder cae a
   composeDoc()/scanFields() sin romper nada.
   ----------------------------------------------------------- */
...
/* -----------------------------------------------------------
   2) Cliente del onboarding — SDIntent → claude-proxy
   Interpreta el TEXTO LIBRE del cliente en el chat y devuelve su intención
   estructurada (tipo de firma, volumen, frecuencia, IA, equipo).
   ...
   - Fallback: heurística local por palabras clave (sin IA) para que el chat
     igual pueda GUIAR al cliente si el proxy no está disponible.
   ----------------------------------------------------------- */
(function (global) {
